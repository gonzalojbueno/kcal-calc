...
function render() {
  ...
  const selectedDate = getSelectedDateCL();
  ...
  const caloriePercent =
    Math.min((totalGeneral / calorieGoal) * 100, 100);

  const proteinPercent =
    Math.min((totalProtein / proteinGoal) * 100, 100);

  const fatPercent =
    Math.min((totalFat / fatGoal) * 100, 100);

  let calorieBarColor = "bg-success";

  if (totalGeneral > calorieGoal) {
    calorieBarColor = "bg-danger";
  } else if (caloriePercent >= 90) {
    calorieBarColor = "bg-warning";
  }
  ...
  totalDiv.innerHTML = `
    ...
    <div class="progress mb-3" style="height:28px;">
      <div
        class="progress-bar ${calorieBarColor}"
    ...
    <div class="progress mb-3" style="height:28px;">
      <div
        class="progress-bar progress-bar-striped ${proteinBarColor}"
